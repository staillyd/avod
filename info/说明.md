...
- 存在问题
  - obj_utils.get_lidar_point_cloud()读取velo坐标系下的点云(x,y,z,r),并保留在相机前方、且在图片内的三维点云point_cloud，该点云坐标转换到了相机坐标。
  - index=23时,
    - 原始数据，在velo坐标系下
      - x大概~(-79,74.8) 
      - y大概~(-26,57)
      - z大概~(-7,2.9)  #?? 怎么感觉是反的#单位不是m,并不知道是什么
    - 转换到相机坐标
      - x大概~(-57.7,26.4) 
      - y大概~(-3.8,6.6) #?? 怎么感觉是反的
      - z大概~(-79,74.5)
    - 截取在相机前的点云
      - x大概~(-47.6,21) 
      - y大概~(-1.39,3.67) #?? 怎么感觉是反的
      - z大概~(2.9,74.5)
    - 可以投影到图像里的点云(相机坐标系)
      - x大概~(-32,17) 
      - y大概~(-1.37,3) #?? 怎么感觉是反的
      - z大概~(4.5,74.5)
    - kitti_utils.create_slice_filter()里:主要看y，见[plane](#plane)
      ...
      - point_cloud[:,road_filter]保留plane上方plane_offset_dist=0.2以下的点云
        ...
        - y大概~(1.2,3) #?? 怎么感觉是反的
        - z大概~(5,59.8)
      - point_cloud[:,slice_filter]保留plane上方0.2以上，2以下的点云
        - x大概~(-32,17) 
        - y大概~(-0.659,1.9) #plane是斜的，不是严格的(0,-1,0)  #?? 怎么感觉是反的
        - z大概~(4.5,68.7)
...
- [-7.051739e-03 -9.997791e-01 -1.980150e-02] 水平面的单位法向量. 几乎和 [0 -1 0] 相等，说明地面平坦。根据obj_utils.get_road_plane()里描述的+y是向下的，也许是和cam所在坐标系一致。这里法向量向上。
...
# 聚类
label_cluster.get_clusters()对训练集里的每个样本对应类的所有目标的长宽高进行聚类，保存x个聚类中心点和x个标准差。
- 得到每个class聚类后的长宽高
...
- anchor_info: mini_batch_preprocessor._calculate_anchors_info()
...
# code tips
- 多维的排序、多维的去重:voxel_grid_2d.voxelize_2d()函数里
- 点云floor->int，然后多维排序、去重进行量化:voxel_grid_2d.voxelize_2d()函数里
- np.sum等有axis的理解:
